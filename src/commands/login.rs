//! `stackoverflow login`. Authenticates with Apify via API token, opening the console
...
pub fn run(args: LoginArgs) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists."))
            .fix(format!("Use --force to replace its key: stackoverflow login {existing} --force")));
    }
...
fn prompt_login_key(name: &str, no_browser: bool) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API token given and no terminal to prompt on.")
            .fix(format!("pbpaste | stackoverflow login {name} --api-key-stdin")));
    }
    ...
    loop {
        ...
        eprintln!("API token cannot be empty. Paste your token from {API_KEYS_URL}");
    }
}
