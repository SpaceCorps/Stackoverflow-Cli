...
//!
//! Stored in `config.yaml` within the user configuration directory.
...
pub struct AccountConfig {
    /// Identity or label for this account.
    pub identity: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("STACKOVERFLOW_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("stackoverflow-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("stackoverflow-cli")
        } else {
            std::env::var_os("XDG_CONFIG_HOME")
                .filter(|v| !v.is_empty())
                .map(PathBuf::from)
                .unwrap_or_else(|| home.join(".config"))
                .join("stackoverflow-cli")
        }
    };
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: stackoverflow accounts add <name>", path.display()))
    })
}
...
/// Not reentrant. Keep network calls outside it.
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another stackoverflow process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn config_serialization_roundtrip() {
        ...
        c.accounts.insert(
            "work".into(),
            AccountConfig { identity: "token-1234".into(), added_at: "2026-01-01T00:00:00Z".into() },
        );
        let yaml = serde_norway::to_string(&c).unwrap();
        let loaded: Config = serde_norway::from_str(&yaml).unwrap();
        let (name, acct) = loaded.find("work").unwrap();
        assert_eq!(name, "work");
        assert_eq!(acct.identity, "token-1234");
    }
}
